...
def analizza_con_ai(tipo, dati_testo):
    ...
    if tipo == "airbus":
        prompt_sistema = ("Sei un analista di intelligence aeronautica e industriale. Ricevi dati sui voli rilevati sopra Tolosa (sede di Airbus). Analizza i dati e fornisci un briefing predittivo professionale: interpreta l'attività aerea, ipotizza cosa sta succedendo nello stabilimento e fai una previsione sull'attività produttiva di Airbus nelle prossime settimane. Rispondi in italiano, in modo conciso e strutturato con titoli e punti chiave.")
        prompt_utente = f"Ecco i dati dei voli rilevati sopra Tolosa in questo momento:\n\n{dati_testo}\n\nFornisci il tuo briefing predittivo."
    else:
        prompt_sistema = ("Sei un analista di intelligence tecnologica e brevettuale. Ricevi un elenco di pubblicazioni scientifiche e brevetti recenti di Ferrari. Analizza i titoli e le date, individua i pattern tecnologici ricorrenti e fai una previsione su quale direzione tecnologica sta prendendo Ferrari e quale potrebbe essere il prossimo prodotto o innovazione. Rispondi in italiano, in modo conciso e strutturato con titoli e punti chiave.")
        prompt_utente = f"Ecco i brevetti/pubblicazioni recenti di Ferrari:\n\n{dati_testo}\n\nFornisci il tuo briefing predittivo."
    headers = {"Authorization": f"Bearer {api_key}", "Content-Type": "application/json"}
    payload = {"model": "deepseek-chat", "messages": [{"role": "system", "content": prompt_sistema}, {"role": "user", "content": prompt_utente}], "temperature": 0.7, "max_tokens": 800}
    try:
        risposta = requests.post("https://api.deepseek.com/v1/chat/completions", headers=headers, json=payload, timeout=30 )
